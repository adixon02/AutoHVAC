...
if __name__ == "__main__":
    ...
    test_zips = [
        "99019",  # Spokane area (existing data)
        "10001",  # NYC 
        "90210",  # Beverly Hills
        "33101",  # Miami
        "60601",  # Chicago
    ...
    print(f"\n🔍 Search Test: 'New York'")
    search_results = db.search_zip_codes("New York", limit=5)
    for result in search_results:
        print(f"  {result['zip_code']} - {result['city']}, {result['state_abbr']} "
              f"(Zone: {result['climate_zone'] or 'N/A'})")
